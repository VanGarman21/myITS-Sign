...
export default function EditorTandaTanganSendiri({
  ...
  const handleSubmitPassphrase = async () => {
    ...
    if (!pdfData || !nik || !spesimen || signatureWidgets.length === 0) return;
    setSignLoading(true);
    try {
      ...
      if (pendingLockId) {
        setSignatureWidgets((prev) =>
          prev.map((w) =>
            w.id === pendingLockId ? { ...w, isLocked: true } : w
          )
        );
      }
  ...
  useEffect(() => {
    const embedIfLocked = async () => {
      if (!pdfData || !allLocked || downloadUrl || !signatureImage) return;
  ...
  }, [allLocked, pdfData, signatureWidgets, signatureImage]);
